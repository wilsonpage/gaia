...
Across the code base you may see code like this:
...
Here we are making a copy of a prototype method, bound to the given context (`this`). We then store this copy on the instance of the class. We do this so that we can pass the function as a callback to an event handler and inside the function `this`, will still refer to an instance of `MyClass` and not `window`, `HTMLButtonElement`, etc.

```js
foo.on('thinghappened', this.enableButtons);
```

So why not do it like this...?

```js
foo.on('thinghappened', this.enableButtons.bind(this));
```

..because if we need to unbind the function, this code will fail:

```js
foo.off('thinghappened', this.enableButtons);
```

I agree this is a little verbose, so maybe we should look at implementing a cleaner way of doing this. Underscore provides `_.bindAll()` which takes care of copying bound versions of prototype methods onto the instance.
